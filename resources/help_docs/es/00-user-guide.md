...
**Orquestador** opcional: el chat solo al orquestador, un LLM al orquestador, un **Canal** del orquestador al canal de cada agente y **Mensaje** del orquestador a **Fin**. Es la única voz del chat de la ejecución, hace preguntas y llama a los agentes de uno en uno. Los textos de los agentes y las órdenes internas no aparecen como burbujas. Cada agente guarda su propio texto. Un agente con herramienta recibe el texto del autor y la lista de archivos que ya escribió. El orquestador solo ve un resumen breve. Si un agente tiene una herramienta y no la llama, o la llamada falla, la tarea sigue abierta. Ese trabajo no se anuncia como hecho en el chat. El chat sigue abierto hasta que él termina la ejecución. Sin orquestador cada agente es su propia cadena por mensaje y transferencia. Un agente está en el canal o en la cadena, nunca en ambos.
